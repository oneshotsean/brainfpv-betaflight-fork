...
void brainFpvRgbLedTimerInit(void)
{
    ...
    for (int i=0; i < 3; i++) {
        ...
        pwmOutputConfig(&rgbLedChannels[i].channel, timerHardware, (RGB_LED_PWM_PERIOD * RGB_LED_PWM_FREQ_KHZ * 1000), RGB_LED_PWM_PERIOD, 0, 0);
    }

    // pwmOutputConfig() starts the PWM channel and the time base itself, so the
    // timer no longer needs to be started here.
}
